...
let system = ActorSystem.Create("GossipSimulator", Configuration.defaultConfig())

let random = Random()

// stating of the program / main entry
// taking all the required arguments from the command line
// starting with index 1 since index 0 will have the file name
let args = fsi.CommandLineArgs
let numberOfNodes = args.[1] |> int
let topology = args.[2]
let algorithm = args.[3]

// global variable to store nearest perfect cube for the given number of nodes 
// used in 3D and Imp 3D topology
let mutable cubeNodes = -1;

// type of information needed to be stored
type Information = 
...
// globally declaring a list of list to store neighbors for each node in the network
...
// creating the boss actor system
let boss = spawn system "boss" BossActor

// sending message / input to the boss actor
boss <? InputToAlgorithm (numberOfNodes, algorithm, neighbours)
